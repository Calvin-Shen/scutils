...
- (instancetype)init
{
    ...
    if (self) {
        // 创建托管对象模型
        NSString *path = [[NSBundle mainBundle] pathForResource:@"SCCoreData" ofType:@"momd"];
        ...
        [_coordinator addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:[self dataBasePath] options:nil error:nil];
        // 创建托管对象上下文
...
+ (void)destory{
    NSString *filePath = [kPathDocument stringByAppendingPathComponent:@"CoreData"];
...
+ (__kindof NSManagedObject *)getModelWithEntityName:(NSString * _Nonnull)entityName{
    NSManagedObject *object = [NSEntityDescription insertNewObjectForEntityForName:entityName inManagedObjectContext:[SCCoreDataManager shareInstance].objectContext];
    return object;
}
...
+ (BOOL)deleteByEntityName:(NSString * _Nonnull)entityName
    ...
    // 查询数据
    NSArray *array = [self selectByEntityName:entityName withMaching:searchString WithAttribute:attribute sortingBy:attribute isAscending:YES];
    if (array.count) {
...
+ (BOOL)updateManagedObject:(__kindof NSManagedObject *)managedObject{
    [[SCCoreDataManager shareInstance].objectContext refreshObject:managedObject mergeChanges:YES];
    return [SCCoreDataManager save];
}
...
+ (NSArray *)selectByEntityName:(NSString * _Nonnull)entityName
    ...
    NSEntityDescription *entity = [NSEntityDescription entityForName:entityName inManagedObjectContext:[SCCoreDataManager shareInstance].objectContext];
    // 创建fetch请求
    ...
    if (!SCStrIsEmpty(sortAttribute)) {
        // 排序
        NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:sortAttribute ascending:ascending selector:nil];
    ...
    NSError *error = nil;
    NSFetchedResultsController *fetchdController = [[NSFetchedResultsController alloc] initWithFetchRequest:fetchRequest managedObjectContext:[SCCoreDataManager shareInstance].objectContext sectionNameKeyPath:nil cacheName:nil];
    // 执行获取操作
 ...
 */
- (NSURL *)dataBasePath{
    NSString *dbFolder = [kPathDocument stringByAppendingPathComponent:@"CoreData"];
